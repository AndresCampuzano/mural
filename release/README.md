...
- [x] Implement Korean and Japanese modules, language/subtitle onboarding, versioned AI consent, local backups and the existing conversation controls.
...
| Final device checks | Test the candidate on an iPhone: microphone denial, offline/failing API requests, interruptions, cellular use, reset, meanings, export/import and deletion. Review pronunciation and corrections for Korean and Japanese with proficient speakers, including Japanese romaji readings and speech levels. Automated live checks do not establish teaching quality. |
